  ...
  const handleRestrict = async () => {
    ...
    try {
      ...
      if (response.status == 'success') {
        toast.success(
          isActive
            ? 'Account restricted successfully'
          ...
          <AlertDialogHeader className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <AlertDialogDescription className="text-sm text-muted-foreground/80 font-medium leading-relaxed">
                {isActive ? (
                  <>
                    This will immediately revoke all administrative protocols
                    for{' '}
                    <span className="text-foreground font-bold">
                      @{displayName}
                    </span>
                    . The entity will be unable to access system cores until
                    manually reinstated.
                  </>
                ) : (
                  <>
                    This will re-initialize system protocols for{' '}
                    <span className="text-foreground font-bold">
                      @{displayName}
                    </span>
                    . The entity will regain complete access to their assigned
