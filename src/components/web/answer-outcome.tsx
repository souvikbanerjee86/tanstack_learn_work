...
import {
  AiVoiceFraudPanel,
  audioAnalysisQueryOptions,
} from './ai-voice-fraud-panel'
import {
  getInterviewAnswersList,
  getInterviewSessionInfo,
  interviewEvaluate,
} from '@/lib/server-function'
import { cn } from '@/lib/utils'

export const interviewAnswerQueryOptions = (email: string, job_id: string) =>
  queryOptions({
    queryKey: ['candidates', email, job_id],
    queryFn: () =>
      getInterviewAnswersList({ data: { candidate: email, job_id: job_id } }),
    staleTime: 1000 * 60 * 5, // 5 minutes cache
    gcTime: 1000 * 60 * 15,
  })

export const interviewSessionInfoQueryOptions = (
  email: string,
  session_id: string,
) =>
  queryOptions({
    ...
    queryFn: () =>
      getInterviewSessionInfo({
        data: { user_id: email, app: 'app', session_id: session_id },
      }),
    enabled: Boolean(email && session_id),
...
export function AnswerOutcome({
  ...
  const sessionId = answers.data[0]?.session_id || ''
  const { data: sessions } = useQuery(
    interviewSessionInfoQueryOptions(email, sessionId),
  )

  // Fetch audio, movement and voice fraud data for the report
  const { data: voiceAnswersData } = useQuery(
    interviewVoiceAnswerQueryOptions(email, id),
  )
  const { data: movementDataResult } = useQuery(
    movementDetectionDetailsQueryOptions(email, id),
  )
  const { data: voiceFraudAnalysis } = useQuery(
    audioAnalysisQueryOptions(email, id),
  )

  const confirmEvaluation = async (data: {
    verdict: string
    feedback: string
  }) => {
  ...
  const movementAnomaliesTotal = movementDataResult?.data
    ? movementDataResult.data.length
    : 0
  ...
  const isAiVoiceDetected = Boolean(
    ...
    (voiceFraudAnalysis.ai_generated_count > 0 ||
      ...
      voiceFraudAnalysis.overall_conclusion
        ?.toLowerCase()
        .includes('suspicious')),
  )
      ...
      <IntegrityTrustGauge
        ...
        voiceFraudStatus={
          voiceFraudAnalysis?.total_audios_analyzed
            ? {
                isAiGenerated: isAiVoiceDetected,
              ...
              <Card className="lg:col-span-3 shadow-xl border border-border/60 overflow-hidden rounded-[2rem] bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl flex flex-col justify-between">
                ...
                <div className="bg-zinc-900 text-white p-6 sm:p-8 flex justify-between items-start gap-4 relative overflow-hidden">
                  ...
                  <div className="flex flex-col items-center justify-center bg-white/10 rounded-2xl p-3 sm:p-4 min-w-[80px] sm:min-w-[90px] backdrop-blur-md border border-white/10 shadow-lg relative z-10 shrink-0">
                    <span className="text-[9px] font-black uppercase tracking-widest text-zinc-300 mb-0.5">
                      Score
                    </span>
                    <span
                      className={cn(
                ...
                <CardContent className="p-6 sm:p-8 space-y-6">
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[9px] font-mono text-muted-foreground/60 border-border/40"
                      >
                        Gemini Engine
                      </Badge>
                    </div>
                  ...
                  <CardContent className="p-5 space-y-5">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <Badge
                          variant="outline"
                          className="text-[10px] font-black uppercase mt-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20"
                        >
                    ...
                    <div className="space-y-3 pt-1">
                      ...
                      <div className="space-y-2.5">
                        <CheckItem
                          ...
                          status={data.answer_evaluation}
                        />
                        <CheckItem
                          label="Identity Match"
                          status={data.text_evaluation}
                        />
                        <CheckItem
                          label="Voice Biometrics"
                          status={data.voice_evaluation}
                        />
                      </div>
                    </div>
                  </CardContent>
