import { Briefcase, CheckCircle2, AlertCircle, FileText, User, Loader2 } from "lucide-react";
...
export function CandidateResultCard({ data, selectedItems, handleCheckedChange, downlaodUrl, isOpen, setIsOpen, isDownloading, fileUrl }: { data: CandidateMatch, selectedItems: string[], handleCheckedChange: (id: string, checked: boolean) => void, downlaodUrl: (url: string) => void, isOpen: boolean, setIsOpen: React.Dispatch<React.SetStateAction<boolean>>, isDownloading: boolean, fileUrl: string }) {
    console.log(data)
    return (
        <>
            <Card className="w-full border-l-4 border-l-primary shadow-md hover:shadow-lg transition-shadow">
                <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="">
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1 uppercase tracking-wider">Source: {data.source_ref.substring(data.source_ref.indexOf("uploads/"))}</p>
                        </div>
                    </div>
                </CardHeader>
                ...
                <CardFooter className="bg-muted/30 pt-4 flex justify-between">
                    ...
                    <Button
                        ...
                        size="sm"
                        variant="outline"
                        className="gap-2 rounded-full hover:bg-primary hover:text-primary-foreground transition-all duration-300 shadow-sm hover:shadow"
                        onClick={(e) => {
                            e.preventDefault()
