  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          className="h-8 w-8 p-0 hover:bg-muted/80 data-[state=open]:bg-muted rounded-lg"
        >
      ...
      <DropdownMenuContent
        align="end"
        className="w-48 rounded-xl shadow-xl border-muted-foreground/10"
      >
        <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70 px-4 py-2">
          System Control
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
        <DropdownMenuItem
...
export const columns: Array<ColumnDef<JobDetail>> = [
  {
    accessorKey: 'job_id',
    header: () => (
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2 text-sm text-muted-foreground font-medium opacity-60">
        ...
        <span className="tabular-nums whitespace-nowrap">
          {format(row.getValue('start_date'), 'MMM dd, yyyy')}
        </span>
      </div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2 text-sm text-muted-foreground font-medium opacity-60">
        <Calendar className="h-3.5 w-3.5" />
        <span className="tabular-nums whitespace-nowrap">
          {format(row.getValue('end_date'), 'MMM dd, yyyy')}
        </span>
      </div>
    ),
  },
