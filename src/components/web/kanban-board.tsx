...
const KANBAN_COLUMNS: Array<ColumnConfig> = [
  {
    id: 'Active',
  ...
  },
  {
    ...
    badgeBg:
      'bg-zinc-100 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-200 border-zinc-300 dark:border-zinc-700',
...
export function KanbanBoard({
  ...
  const groupedJobs = useMemo(() => {
    const groups: Record<'Active' | 'Inactive' | 'Archived', Array<JobDetail>> = {
      Active: [],
      Inactive: [],
      Archived: [],
    }
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3 pb-3 border-b border-zinc-200 dark:border-zinc-800">
              <div className="flex items-start gap-3 min-w-0 flex-1">
                <div
                  className={cn(
                    'h-8 w-8 rounded-xl flex items-center justify-center shrink-0 border mt-0.5 shadow-xs',
                    col.badgeBg,
                  )}
                >
              ...
              <Badge
                variant="outline"
                className={cn(
                  'text-xs font-mono font-bold px-2.5 py-1 rounded-full border shrink-0',
                  col.badgeBg,
                )}
              >
                {columnJobs.length}
                ...
                <div className="flex flex-col items-center justify-center p-8 text-center rounded-2xl border-2 border-dashed border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/40 my-auto space-y-2">
                  ...
                  <div>
                    <p className="text-xs font-bold text-zinc-800 dark:text-zinc-200">
                      No {col.id.toLowerCase()} positions
                    </p>
