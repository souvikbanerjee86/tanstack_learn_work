...
export function AddMultipleCandidatesDialog({ open, onOpenChange }: AddMultipleCandidatesDialogProps) {
    ...
    const processFiles = (files: FileList | File[]) => {
        const newItems: ResumeFileItem[] = Array.from(files).map((file) => ({
        ...
        toast.info(`Added ${newItems.length} resume file${newItems.length > 1 ? "s" : ""}`)
    }
    ...
    const handleStartBatchUpload = async () => {
        ...
        try {
            ...
            const failedMap = new Map((response.failed_files || []).map((f) => [f.filename, f.reason]))
            ...
            setResumeFiles((prev) =>
                prev.map((item) => {
                    if (uploadedSet.has(item.file.name)) {
                        ...
                        return { ...item, status: "error", errorMessage: failedMap.get(item.file.name) }
                    } else {
                        return { ...item, status: "success" }
                    }
                })
            ...
            if (response.uploaded_files && response.uploaded_files.length > 0) {
                ...
                toast.success(`Successfully uploaded ${response.uploaded_files.length} resume${response.uploaded_files.length > 1 ? "s" : ""}!`)
            }

            if (response.failed_files && response.failed_files.length > 0) {
                toast.error(`${response.failed_files.length} file${response.failed_files.length > 1 ? "s" : ""} failed to upload`)
            } else {
            ...
            setResumeFiles((prev) =>
                prev.map((item) => ({ ...item, status: "error", errorMessage: err?.message || "Upload failed" }))
            )
        } finally {
    ...
    const allCompleted = resumeFiles.length > 0 && resumeFiles.every((f) => f.status === "success")
            ...
            <DialogContent className="sm:max-w-none w-[94vw] md:w-[75vw] lg:w-[50vw] lg:max-w-[50vw] max-h-[92vh] flex flex-col p-0 gap-0 rounded-3xl border border-border/60 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-2xl shadow-2xl overflow-hidden">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <Badge variant="outline" className="border-indigo-500/30 text-indigo-600 dark:text-indigo-400 bg-indigo-500/10 text-[9px] font-black uppercase tracking-widest">
                                        Bulk Upload
                                    </Badge>
                                </div>
                                <DialogDescription className="text-xs md:text-sm text-muted-foreground font-medium flex items-center gap-1.5">
                                    <Sparkles className="h-3.5 w-3.5 text-indigo-500 shrink-0" />
                                    Select the job position and attach candidate resumes for bulk processing.
                                </DialogDescription>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
                    ...
                    <div className="bg-muted/30 border border-border/50 rounded-2xl p-4 md:p-5 space-y-3">
                        ...
                        <Select value={selectedJobId} onValueChange={handleSelectJob} disabled={isProcessing}>
                            <SelectTrigger className="h-12 rounded-xl bg-background border-border/60 focus:ring-indigo-500/20">
                                <SelectValue placeholder={isLoadingJobs ? "Loading active positions..." : "Select position for these resumes..."} />
                            </SelectTrigger>
                            <SelectContent className="rounded-xl border-border shadow-xl max-h-60">
                                {jobDetails?.data && jobDetails.data.length > 0 ? (
                                    jobDetails.data.map((job) => (
                                        <SelectItem key={job.job_id} value={job.job_id} className="rounded-lg py-2.5">
                    ...
                    >
                        ...
                        <h4 className="text-base font-bold tracking-tight">
                            Drop multiple resumes here, or <span className="text-indigo-600 dark:text-indigo-400 underline underline-offset-4">browse files</span>
                        </h4>
                        <p className="text-xs text-muted-foreground mt-1 max-w-sm">
                            Supports PDF and DOCX files. Multiple files can be selected at once.
                        </p>
                    </div>
                                                ...
                                                <div className="flex items-center gap-3 shrink-0">
                                                    {item.status === "uploading" && (
                                                        <div className="flex items-center gap-1.5 text-xs text-indigo-500 font-medium">
                                                            ...
                                                            <span className="hidden sm:inline text-[10px]">Uploading</span>
                                                        </div>
                                                    )}
                                                    {item.status === "success" && (
                                                        <div className="flex items-center gap-1 text-xs text-emerald-500 font-medium">
                                                            <CheckCircle2 className="h-4 w-4" />
                                                            <span className="hidden sm:inline text-[10px]">Uploaded</span>
                                                        </div>
                                                    )}
                                                    {item.status === "error" && (
                                                        <div className="flex items-center gap-1 text-xs text-rose-500 font-medium" title={item.errorMessage}>
                                                            <AlertCircle className="h-4 w-4" />
                                                            <span className="text-[10px]">{item.errorMessage || "Failed"}</span>
                                                        </div>
                                                    )}
                ...
                <DialogFooter className="p-6 border-t border-border/40 bg-muted/20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="text-xs text-muted-foreground font-medium">
                        {resumeFiles.length > 0 ? (
                            <span>
                                <strong className="text-foreground">{resumeFiles.length}</strong> resume{resumeFiles.length !== 1 ? "s" : ""} selected
                            </span>
                        ) : (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button
                            ...
                            disabled={isProcessing || resumeFiles.length === 0 || !selectedJobId || allCompleted}
