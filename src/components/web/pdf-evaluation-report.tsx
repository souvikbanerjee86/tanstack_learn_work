...
import type { EvaluationData, InterviewRecord, UserMovementData } from '@/lib/types'
...
const AudioWaveIcon = () => (
  <Svg width={12} height={12} viewBox="0 0 24 24">
    <Path
      d="M2 10v4M6 6v12M10 3v18M14 8v8M18 5v14M22 10v4"
      stroke={colors.primary}
      strokeWidth="2.5"
      strokeLinecap="round"
      fill="none"
...
interface InterviewPDFReportProps {
  ...
  evaluation?: string
  feedback?: string
  answers?: Array<EvaluationData>
  voiceAnswers?: Array<InterviewRecord>
  movementData?: Array<UserMovementData>
}
...
export function InterviewPDFReport({
  ...
  const proctorRisk =
    totalInfractions === 0
      ? { label: 'Low Risk', color: colors.emerald, bg: colors.emeraldBg }
      : totalInfractions <= 3
        ? { label: 'Moderate Risk', color: colors.amber, bg: colors.amberBg }
        : { label: 'High Risk', color: colors.rose, bg: colors.roseBg }
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerBanner}>
          <View style={{ flexDirection: 'column', gap: 2 }}>
            ...
            <Text style={styles.brandSubtitle}>
              Forensic Evaluation & Multi-Dimensional Candidate Audit
            </Text>
          </View>
          ...
          <View style={styles.metaCol}>
            ...
            <Text
              style={[
                ...
                {
                  color:
                    evaluation === 'EVALUATED' ? colors.emerald : colors.amber,
                },
              ]}
            >
              {evaluation}
        ...
        <View style={styles.dashboardGrid}>
          ...
          <View style={styles.dashCard}>
            ...
            <Text style={styles.dashCardValue}>
              {totalScore}{' '}
              <Text style={{ fontSize: 9, color: colors.slateMuted }}>
                / {answers.length * 10}
              </Text>
            </Text>
            <View
            ...
            >
              <Text
                style={[styles.dashPillText, { color: classification.color }]}
              >
            ...
            <View style={styles.progressBarBg}>
              <View
                style={[
                  ...
                  {
                    width: `${Math.min(100, Math.max(0, (avgScoreNum / 10) * 100))}%`,
                    backgroundColor: colors.primary,
                  },
                ]}
              />
          ...
          <View style={styles.dashCard}>
            ...
            <Text
              style={[
                ...
                {
                  color:
                    voiceConclusion === 'Human' ? colors.emerald : colors.rose,
                  fontSize: 13,
                },
              ]}
            >
            ...
            <View style={styles.progressBarBg}>
              <View
                style={[
                  styles.progressBarFill,
                  {
                    width: `${voiceConfidence}%`,
                    backgroundColor:
                      voiceConclusion === 'Human'
                        ? colors.emerald
                        : colors.rose,
                  },
                ]}
          ...
          <View style={styles.dashCard}>
            ...
            <Text style={[styles.dashCardValue, { color: proctorRisk.color }]}>
              {totalInfractions}{' '}
              <Text style={{ fontSize: 9, color: colors.slateMuted }}>
                Flags
              </Text>
            ...
            <View
              style={[styles.dashPill, { backgroundColor: proctorRisk.bg }]}
            >
              <Text style={[styles.dashPillText, { color: proctorRisk.color }]}>
                {proctorRisk.label}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.sectionHeader} wrap={false}>
          ...
          <Text
            style={[
              styles.sectionBadge,
              {
                backgroundColor: feedback ? colors.emeraldBg : colors.amberBg,
                color: feedback ? colors.emerald : colors.amber,
              },
            ]}
          >
        ...
        {feedback ? (
          ...
          <View style={styles.adminPendingCard} wrap={false}>
            <Text style={styles.adminPendingText}>
              Evaluation of Admin is Pending
            </Text>
          </View>
        )}
                    ...
                    <Text
                      style={
                        item.answer_evaluation
                          ? styles.diagStatusPass
                          : styles.diagStatusFail
                      }
                    >
        ...
        <View style={styles.sectionHeader} wrap={false}>
          <View style={styles.sectionTitleRow}>
            ...
            <Text style={styles.sectionTitle}>
              Voice Audit & Forensic Deep-Dive
            </Text>
          </View>
        ...
        {voiceAnswers.length === 0 ? (
          ...
          voiceAnswers.map((voice, idx) => {
            ...
            const confidence = (
              (voice.analysis_result?.confidence_score ?? 0) * 100
            ).toFixed(0)
              ...
              <View key={idx} style={styles.voiceCard} wrap={false}>
                <View style={styles.voiceHeader}>
                  ...
                  >
                    ...
                    >
                      Conclusion:{' '}
                      {voice.analysis_result?.conclusion || 'Unverified'}
                    </Text>
                  </View>
                ...
                <Text style={styles.voiceReasoning}>
                  "
                  {voice.analysis_result?.reasoning ||
                    'No forensic reasoning provided.'}
                  "
                </Text>

                <Text style={styles.voiceMeta}>
                  GCS URI: {voice.gcs_uri || 'N/A'} • Timestamp:{' '}
        ...
        <View style={styles.sectionHeader} wrap={false}>
          <Text style={styles.sectionTitle}>
            Proctoring & Incident Timeline Log
          </Text>
        ...
        {!movementSummary ||
        !movementSummary.events ||
        movementSummary.events.length === 0 ? (
          <Text
            style={{
              fontStyle: 'italic',
              color: colors.emerald,
          ...
          <View style={styles.table} wrap={false}>
            ...
            {movementSummary.events.map((evt, idx) => {
              ...
              const category = isFace
                ? 'Face Tracking'
                : isFocus
                  ? 'Focus Lost'
                  : 'Violation'
              const categoryBg = isFace
                ? colors.amberBg
                : isFocus
                  ? colors.primaryLight
                  : colors.roseBg
              const categoryColor = isFace
                ? colors.amber
                : isFocus
                  ? colors.primary
                  : colors.rose
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerText}>
            EazyAI Intelligence Systems • Automated Candidate Audit Report •
            Confidential
          </Text>
          <Text
