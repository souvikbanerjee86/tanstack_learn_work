...
export const audioAnalysisQueryOptions = (candidateEmail: string, jobId: string) =>
    queryOptions({
...
export const AiVoiceFraudPanel: React.FC<AiVoiceFraudPanelProps> = ({
    ...
    const { data: analysis, isLoading, isError } = useQuery(
        audioAnalysisQueryOptions(candidateEmail, jobId)
    );

    if (isLoading) {
        return (
            <Card className={cn("p-6 rounded-3xl border border-border/60 bg-white/40 dark:bg-zinc-900/40 backdrop-blur-xl shadow-lg animate-pulse", className)}>
        ...
        >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 relative z-10">
                <div className="flex flex-col items-center justify-center p-3.5 rounded-2xl bg-white/60 dark:bg-zinc-900/60 border border-border/60 text-center shadow-xs">
                    ...
                    <span className="text-[10px] font-bold text-muted-foreground uppercase">Audios Analyzed</span>
                    <span className="text-base font-black font-mono text-foreground mt-0.5">{total_audios_analyzed}</span>
                </div>

                <div className="flex flex-col items-center justify-center p-3.5 rounded-2xl bg-white/60 dark:bg-zinc-900/60 border border-border/60 text-center shadow-xs">
                    <Bot className="h-4 w-4 text-rose-500 mb-1" />
                    <span className="text-[10px] font-bold text-muted-foreground uppercase">AI TTS Flagged</span>
                    <span className={cn("text-base font-black font-mono mt-0.5", ai_generated_count > 0 ? "text-rose-600 dark:text-rose-400" : "text-foreground")}>
                        {ai_generated_count}
                    </span>
                </div>
                ...
                <div className="flex flex-col items-center justify-center p-3.5 rounded-2xl bg-white/60 dark:bg-zinc-900/60 border border-border/60 text-center shadow-xs">
                    ...
                    <span className="text-[10px] font-bold text-muted-foreground uppercase">Human Voice</span>
                ...
                <div className="flex flex-col items-center justify-center p-3.5 rounded-2xl bg-white/60 dark:bg-zinc-900/60 border border-border/60 text-center shadow-xs">
                    ...
                    <span className="text-[10px] font-bold text-muted-foreground uppercase">Model Certainty</span>
            ...
            <div className="space-y-2 relative z-10">
                <div className="flex justify-between items-center text-xs font-semibold text-muted-foreground">
                    ...
                    <span className="font-mono">{avgConfidencePercent}% Average Certainty</span>
                </div>
                ...
                <div className="space-y-3 relative z-10 pt-2 border-t border-border/40">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-foreground">
                            ...
                            <span>Question-by-Question Speech Forensics ({records.length} recordings)</span>
                        </div>
                        <Button
                            variant="ghost"
                            size="sm"
                        ...
                        >
                            <span>{expandedRecords ? "Hide Audio Breakdown" : "View Audio Breakdown"}</span>
                            {expandedRecords ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                        </Button>
                    </div>
                    ...
                    {!expandedRecords && records[0]?.reasoning && (
                        <div className="p-4 rounded-2xl bg-white/70 dark:bg-zinc-900/70 border border-border/60 text-xs font-mono text-muted-foreground leading-relaxed pl-5 border-l-2 border-indigo-500/40">
                            <span className="font-bold text-foreground mr-1">Primary Finding:</span>
                            {records[0].reasoning}
                        </div>
                    )}
                    ...
                    {expandedRecords && (
                        <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                            {records.map((record, index) => {
                                const recordIsAi = record.conclusion.toLowerCase().includes("ai");
