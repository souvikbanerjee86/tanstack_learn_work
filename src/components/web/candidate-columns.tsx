...
export const candidateColumns: Array<ColumnDef<candidate>> = [
  {
    accessorKey: 'email',
    header: () => (
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-sm font-medium opacity-80 truncate max-w-[200px]">
          {row.getValue('email')}
        </span>
      </div>
    ),
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const formatted = format(
        new Date(row.getValue('uploaded_at')),
        'MMM dd, yyyy',
      )
      return (
