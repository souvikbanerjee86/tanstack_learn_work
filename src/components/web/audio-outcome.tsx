...
export const interviewVoiceAnswerQueryOptions = (
  ...
  queryOptions({
    ...
    queryFn: () =>
      getInterviewVoiceAnswersList({
        data: { candidate: email, job_id: job_id },
      }),
...
export function AudioOutcome({ email, id }: { email: string; id: string }) {
  const { data: voiceAnswers } = useSuspenseQuery(
  ...
  const downloadVoiceUrl = async (url: string) => {
    try {
      ...
      setFileUrl('')
      setIsOpen(false)
      const response = await getVoiceDownloadURL({
        data: {
          bucket_name: 'interview_speech_project-e7c52c57-c7d4-407d-b4b',
          file_path: url,
        },
      })
  ...
  return (
    <div className="flex items-center">
      <Sheet>
        ...
        <SheetContent
          side="right"
          className="w-full sm:max-w-xl md:max-w-2xl bg-white/95 dark:bg-zinc-950/95 backdrop-blur-2xl border-l border-border/60 flex flex-col p-0 shadow-2xl"
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <SheetDescription className="text-xs text-muted-foreground font-medium">
                  Acoustic forensics, speech analysis, and AI synthesis
                  detection
                </SheetDescription>
              </div>
            </div>
                ...
                >
                    ...
                    <div className="grid grid-cols-2 gap-4 p-4 rounded-2xl bg-muted/30 border border-border/40">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-base font-mono font-black tabular-nums text-foreground">
                          {(
                            (data.analysis_result?.confidence_score ?? 0) * 100
                          ).toFixed(0)}
                          %
                        </p>
                      </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-4 rounded-2xl bg-muted/20 border border-border/30 text-xs text-foreground/85 leading-relaxed italic">
                      "
                      {data.analysis_result?.reasoning ||
                        'No anomalies or synthetic pitch patterns detected during speech sample.'}
                      "
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <h4 className="flex items-center gap-2 text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                      <Database className="w-3.5 h-3.5 text-indigo-500" /> Audio
                      Storage Details
                    </h4>
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      if (data.gcs_uri) {
                        ...
                        downloadVoiceUrl(
                          uploadIdx !== -1
                            ? data.gcs_uri.substring(uploadIdx)
                            : data.gcs_uri,
                        )
                      }
                    }}
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-lg bg-white/95 dark:bg-zinc-950/95 backdrop-blur-2xl border border-border/60 rounded-3xl p-6 shadow-2xl space-y-4">
          ...
          <div className="space-y-4">
            {fileUrl ? (
              <AudioWaveformPlayer
                ...
                title="Acoustic Voice Sample"
                candidateName={email}
              />
            ) : (
              <div className="flex items-center justify-center p-12 bg-muted/20 rounded-2xl border border-border/40">
