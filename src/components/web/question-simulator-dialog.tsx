            ...
            <DialogContent className="sm:max-w-[650px] rounded-3xl p-0 overflow-hidden border border-border/60 bg-zinc-950 text-white shadow-2xl">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-zinc-900/80">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <span className="text-xs font-bold text-zinc-200">Candidate View Simulator</span>
                            <p className="text-[10px] text-zinc-400 font-mono">Live Proctored Session</p>
                        </div>
                    </div>
                ...
                <div className="p-6 sm:p-8 space-y-6 max-h-[75vh] overflow-y-auto no-scrollbar">
                    ...
                    <div className="p-5 rounded-2xl bg-zinc-900 border border-zinc-800 space-y-3">
                        <div className="flex items-center justify-between">
                            <Badge variant="outline" className="text-[10px] font-black uppercase tracking-wider bg-indigo-500/10 text-indigo-400 border-indigo-500/20">
                                Question 1 of 5
                            </Badge>
                            {jobName && (
                        ...
                        <h3 className="text-base sm:text-lg font-bold text-zinc-100 leading-relaxed">
                            {questionText || "Explain your experience with microservice architecture and handling distributed system failures."}
                        </h3>
                    </div>
                    ...
                    <div className="p-5 rounded-2xl bg-zinc-900/60 border border-zinc-800/80 space-y-4">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-zinc-300 flex items-center gap-2">
                                <Mic className={cn("h-4 w-4", isRecording ? "text-rose-400 animate-pulse" : "text-zinc-500")} />
                                {isRecording ? "Listening & Transcribing Speech..." : recordingCompleted ? "Audio Response Captured" : "Click Start to Speak Answer"}
                            </span>
                            {isRecording && (
                        ...
                        <div className="flex items-center justify-center gap-1.5 h-12 bg-zinc-950 rounded-xl px-4 border border-zinc-800/60 overflow-hidden">
                            {Array.from({ length: 28 }).map((_, i) => (
                                <div
                                    ...
                                    style={{
                                        height: isRecording ? `${Math.max(6, (Math.sin(i + timeLeft) * 20 + 22))}px` : undefined,
                            ...
                            >
                                {isRecording ? (
                                    ...
                                    <>
                                        ...
                                        <span>{recordingCompleted ? "Re-record Answer" : "Start Speaking Answer"}</span>
                                    </>
                                )}
                            </Button>
                        </div>
                ...
                <div className="px-6 py-4 border-t border-zinc-800 bg-zinc-900/60 flex items-center justify-between text-xs text-zinc-400">
                    <span className="text-[11px] opacity-75">
                        This modal simulates the interviewee portal rendering and timing rules.
                    </span>
                    <Button
                        variant="outline"
                        size="sm"
