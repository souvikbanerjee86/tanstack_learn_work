...
export const StatusBadge = ({
  ...
  if (disabled) {
    return (
      <Badge
        variant="outline"
        className="rounded-full px-2.5 py-0.5 text-[10px] font-black bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-950/20 dark:text-rose-400 dark:border-rose-900/50 uppercase tracking-tighter"
      >
  ...
  return (
    ...
    >
      {active ? (
        <CheckCircle2 className="h-3 w-3 mr-1 opacity-70" />
      ) : (
        <XCircle className="h-3 w-3 mr-1 opacity-70" />
      )}
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="end"
          className="w-56 rounded-xl shadow-xl border-muted-foreground/10"
        >
          ...
          {isAdmin ? (
            ...
            <DropdownMenuItem
              className="gap-2.5 px-4 py-2.5 cursor-pointer focus:bg-muted focus:text-muted-foreground transition-colors disabled:opacity-50"
              disabled
            >
              <Lock className="h-4 w-4 opacity-70" />
              <div className="flex flex-col gap-0.5">
...
export const getAdminColumns = (
...
): Array<ColumnDef<UserData>> => [
  {
    ...
    cell: ({ row }) => {
      const name = row.getValue('display_name')
      ...
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-muted-foreground font-mono opacity-60 leading-none mt-0.5">
              {row.original.uid}
            </span>
          </div>
        </div>
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <RoleBadge role={row.original.user_role?.role || 'user'} />
    ),
  },
  {
    id: 'status',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (!createdAt)
        return (
          <span className="text-xs text-muted-foreground opacity-40">N/A</span>
        )
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <AdminActions currentUserRole={currentUserRole} rowData={row.original} />
    ),
  },
]
