...
export const movementDetectionDetailsQueryOptions = (
  email: string,
  job_id: string,
) =>
  queryOptions({
    ...
    queryFn: () =>
      getMovementDetectionDetails({
        data: { user_email: email, job_id: job_id },
      }),
...
export function MovementOutCome({ email, id }: { email: string; id: string }) {
  const { data: mockMovementData } = useSuspenseQuery(
    movementDetectionDetailsQueryOptions(email, id),
  )

  if (
    !mockMovementData?.data?.[0]?.events ||
    mockMovementData.data[0].events.length === 0
  ) {
    return null
  }

  const sessionData = mockMovementData.data[0]
  const sortedEvents = [...sessionData.events].sort((a, b) =>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <SheetDescription className="text-xs text-muted-foreground font-medium">
                  Proctoring telemetry, focus shifts, and browser anomaly
                  detection
                </SheetDescription>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="flex items-center gap-2 text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                  <Clock className="w-3.5 h-3.5 text-orange-500" /> Incident
                  Timeline
                </h4>
              ...
              <div className="relative border-l-2 border-border/50 ml-3 space-y-4 pb-2">
                {sortedEvents.map((event, index) => {
                  const isFaceDetection = event.reason
                    .toLowerCase()
                    .includes('face')
                  const isBrowserFocus =
                    event.reason.toLowerCase().includes('browser') ||
                    event.reason.toLowerCase().includes('tab')
