...
interface KeywordRubricInspectorProps {
    ...
    questions: { question?: string; sample_answer?: string; keywords?: string[] }[];
}

export function KeywordRubricInspector({ candidateAnswers, questions }: KeywordRubricInspectorProps) {
    const analysis = useMemo(() => {
        ...
        const defaultTechnicalKeywords = [
            "architecture", "scalability", "concurrency", "performance", "optimization", 
            "security", "microservices", "latency", "async", "cache", "database", 
            "indexing", "docker", "kubernetes", "distributed", "testing", "monitoring",
            "lifecycle", "state management", "api", "rest", "graphql", "clean code",
            "tradeoffs", "reliability", "ci/cd", "observability", "data integrity"
        ];
        ...
        questions.forEach((q) => {
            ...
            if (q.sample_answer) {
                const words = q.sample_answer.toLowerCase().split(/\W+/).filter((w) => w.length > 5);
        ...
        const matchRatio = Math.round((matched.length / Math.max(1, targetKeywords.length)) * 100);
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className="text-[10px] font-mono font-bold bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20">
                                {analysis.matchRatio}% Depth Fit
                            </Badge>
                        </div>
                        <p className="text-xs text-muted-foreground">
                            Semantic keyword presence detected across spoken & transcribed responses.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3 p-4 rounded-2xl bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-medium text-emerald-600/80">Spoken in Answers</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                        {analysis.matched.length > 0 ? (
                            ...
                            <span className="text-xs text-muted-foreground italic">No primary domain keywords matched.</span>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-3 p-4 rounded-2xl bg-zinc-500/5 dark:bg-zinc-500/10 border border-border/60">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-medium text-muted-foreground/60">Expected in Senior Scope</span>
                    </div>
