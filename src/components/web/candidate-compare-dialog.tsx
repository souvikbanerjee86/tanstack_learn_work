...
import type { CandidateMatch, JobDetail, ProfileSearchCritieria } from '@/lib/types'
...
export function CandidateCompareDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-w-5xl! w-[95vw] h-[90vh] max-h-[850px] p-0 rounded-3xl border-border/60 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-2xl shadow-2xl overflow-hidden flex flex-col"
        aria-describedby="candidate-compare-description"
      >
        {/* Header Strip */}
        <div className="px-5 sm:px-6 py-4 border-b border-border/40 bg-muted/20 flex items-center justify-between gap-4 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="h-10 w-10 rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex items-center justify-center border border-indigo-500/20 shadow-sm shrink-0">
            ...
            <div className="min-w-0">
              <DialogTitle className="text-base sm:text-lg font-black tracking-tight text-foreground flex items-center gap-2 flex-wrap">
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] font-mono font-bold bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20"
                >
                  {candidate.matched_score}% MATCH
                </Badge>
              </DialogTitle>
              <p
                id="candidate-compare-description"
                className="text-xs text-muted-foreground font-medium truncate"
              >
                Comparing candidate qualifications directly against job
                requirements
              </p>
            </div>
          </div>
            ...
            >
              <Checkbox
                ...
                onCheckedChange={(checked: boolean) =>
                  onToggleSelect(candidate.candidate_email, checked)
                }
              />
              <Label
                htmlFor={`modal-check-${candidate.candidate_email}`}
                className="text-xs font-bold uppercase tracking-wider cursor-pointer select-none hidden sm:inline"
              >
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border/40 overflow-hidden min-h-0">
          ...
          <div className="flex flex-col h-full min-h-0 overflow-hidden bg-muted/10">
            <div className="px-5 py-3.5 border-b border-border/30 bg-muted/20 flex items-center justify-between gap-3 shrink-0">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {minExperience !== undefined && (
                      <Badge
                        variant="outline"
                        className="bg-background text-xs font-semibold py-1 px-2.5 border-border/60"
                      >
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {requiredSkills.map((s) => (
                          <Badge
                            key={s}
                            variant="secondary"
                            className="text-xs font-medium bg-indigo-500/10 text-indigo-600 dark:text-indigo-400"
                          >
                            {s}
                          </Badge>
                        ))}
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="text-xs sm:text-sm text-foreground/90 leading-relaxed font-normal whitespace-pre-wrap break-words rounded-2xl bg-background/80 dark:bg-zinc-900/60 p-4 sm:p-5 border border-border/40 shadow-xs selection:bg-indigo-500/20">
                      {jobDescription}
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col h-full min-h-0 overflow-hidden bg-background">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground font-medium mt-1">
                      ...
                      <span>•</span>
                      <span>
                        Notice: {candidate.notice_period || 'Immediate'}
                      </span>
                    </div>
                  ...
                  <div className="space-y-2 p-4 rounded-2xl bg-emerald-50/40 dark:bg-emerald-950/20 border border-emerald-500/20">
                    <h4 className="text-[10px] font-black uppercase tracking-wider flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                      <CheckCircle2 className="h-3.5 w-3.5" /> Strong Role
                      Alignments
                    </h4>
                    ...
                    candidate.missing_information.length > 0 && (
                      <div className="space-y-2 p-4 rounded-2xl bg-amber-50/40 dark:bg-amber-950/20 border border-amber-500/20">
                        <h4 className="text-[10px] font-black uppercase tracking-wider flex items-center gap-2 text-amber-600 dark:text-amber-400">
                          <AlertCircle className="h-3.5 w-3.5" /> Areas to
            ...
            <div className="p-4 border-t border-border/40 bg-muted/20 flex items-center justify-between gap-3 shrink-0">
              <Button
                ...
                onClick={() =>
                  onViewResume(
                    candidate.source_ref.substring(
                      candidate.source_ref.indexOf('uploads/'),
                    ),
                  )
                }
              >
