...
export function EmailReaderDialog({ emailItem, open, onOpenChange }: EmailReaderDialogProps) {
    if (!emailItem) return null;

    const senderEmail = emailItem.from_email || emailItem.email || "applicant@example.com";
    const senderName = emailItem.candidate_name || emailItem.name || senderEmail.split('@')[0];
    const jobRole = emailItem.job_name || emailItem.subject || "Open Requisition";
    const receivedDate = emailItem.date || emailItem.created_at || new Date().toISOString();
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[620px] rounded-3xl p-0 overflow-hidden border border-border/60 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-2xl shadow-2xl">
                ...
                <div className="p-6 sm:p-7 space-y-6 max-h-[75vh] overflow-y-auto no-scrollbar">
                    ...
                    <div className="p-4 rounded-2xl bg-muted/40 border border-border/60 space-y-3">
                        <div className="flex items-start justify-between gap-2">
                            <div>
                                <h4 className="font-bold text-sm sm:text-base text-foreground">{senderName}</h4>
                                <p className="text-xs text-muted-foreground font-mono">{senderEmail}</p>
                            </div>
                            <Badge variant="outline" className="text-[10px] font-black uppercase bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20">
                                Synced & Verified
                            </Badge>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-muted-foreground pt-1 border-t border-border/30">
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span>{format(new Date(receivedDate), "MMM dd, yyyy • hh:mm a")}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {emailItem.has_attachment !== false && (
                        <div className="p-3 rounded-xl bg-indigo-500/5 border border-indigo-500/15 flex items-center justify-between">
                            ...
                            <Badge variant="outline" className="text-[10px] font-mono bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20">
                                PDF Document
                            </Badge>
                        </div>
                    )}
                ...
                <div className="p-4 sm:p-6 border-t border-border/40 bg-muted/20 flex items-center justify-between">
                    ...
                    <Link to="/dashboard/candidates/add" search={{ jobId: emailItem.job_id, jobName: emailItem.job_name }}>
                        <Button
                            size="sm"
