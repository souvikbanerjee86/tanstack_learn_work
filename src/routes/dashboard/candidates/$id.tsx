            ...
            <div className="flex flex-col items-center justify-center min-h-[60vh] p-4 text-center space-y-5 animate-in fade-in duration-500">
                ...
                <div className="space-y-1.5 max-w-sm">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                        The candidate record is no longer available in the active session state.
                    </p>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <Link to="/dashboard/candidates">
                        ...
                        >
                            ...
                            <span className="font-bold text-xs uppercase tracking-wider">Back to Candidates</span>
                        </Button>
                    </Link>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <Card className="rounded-[2.5rem] border border-border/60 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl shadow-xl shadow-black/5 overflow-hidden">
                            <CardContent className="p-6 sm:p-8 text-center space-y-6">
                                <div className="relative inline-block mt-2">
                                    ...
                                    {candidateData.candidate_image ? (
                                        <Avatar className="h-28 w-28 rounded-3xl flex items-center justify-center mx-auto border-4 border-background shadow-2xl relative z-10 overflow-hidden">
                                            <AvatarImage src={candidateData.candidate_image} alt={candidateData.name} className="object-cover" />
                                ...
                                <div className="space-y-1">
                                    <h2 className="text-2xl font-black tracking-tight text-foreground">{candidateData.name}</h2>
                                    <p className="text-xs font-semibold text-muted-foreground truncate">{candidateData.email}</p>
                                </div>

                                <Badge variant="outline" className="px-3.5 py-1 rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20 font-mono text-[10px] font-bold">
                                    REF: {candidateData.id ? candidateData.id.substring(0, 10) : "N/A"}
                                </Badge>
                        ...
                        <div className="p-6 rounded-3xl bg-indigo-500/5 border border-indigo-500/10 space-y-3">
                            <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
                                ...
                                <span className="text-xs font-black uppercase tracking-wider">Document Integrity</span>
                            </div>
...
function CVContent({ candidate }: { candidate: candidate }) {
    ...
    const cleanedPath = uploadIndex !== -1 ? resumePath.substring(uploadIndex) : resumePath
                ...
                <CardContent className="p-6 sm:p-8 flex-1 flex flex-col min-h-[700px]">
                        ...
                        {fileUrl ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-full p-12 text-center space-y-4 my-auto">
                                ...
                                <div className="space-y-1 max-w-xs">
                                    <h3 className="text-base font-bold text-foreground">Document Stream Unavailable</h3>
                                    <p className="text-xs text-muted-foreground leading-relaxed">
                                        The preview could not be loaded directly into the frame. Use the fullscreen action above.
                                    </p>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="p-3.5 rounded-2xl bg-muted/20 border border-border/30">
                            <p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground mb-0.5">Database ID</p>
                            <p className="text-xs font-mono text-foreground font-semibold truncate">{candidate.id}</p>
                        </div>
                        <div className="p-3.5 rounded-2xl bg-muted/20 border border-border/30 flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-wider text-muted-foreground mb-0.5">File Format</p>
                                <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400">PDF Application Document</p>
                            </div>
    ...
    return (
        <div className="flex items-center gap-3.5 group min-w-0">
            ...
            <div className="min-w-0 flex-1">
                ...
                <p className="text-xs sm:text-sm font-bold text-foreground tracking-tight truncate">{value}</p>
            </div>
        </div>
    );
}
