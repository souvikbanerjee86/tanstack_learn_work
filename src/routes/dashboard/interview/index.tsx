...
import {
  InterviewActions,
  candidateInterviewEmailColumns,
} from '@/components/web/interview-email-columns'
...
export const interviewQueryOptions = queryOptions({
  ...
  queryFn: () =>
    getInterviewCandidateEmailList({
      data: { limit: null, last_doc_id: null },
    }),
})
    ...
    <div className="relative min-h-screen flex flex-col gap-8 md:gap-10 p-4 md:p-10 lg:p-14 pb-20 bg-transparent overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="relative group">
        ...
        <div className="hidden lg:block relative group p-8 rounded-[2.5rem] border border-border/60 bg-white/50 dark:bg-zinc-950/50 backdrop-blur-xl shadow-2xl shadow-black/5">
          <DataTable
            columns={candidateInterviewEmailColumns}
            data={data.data}
          />
        </div>
  ...
  <Card className="relative overflow-hidden rounded-2xl border border-border/60 bg-white/50 dark:bg-zinc-950/50 backdrop-blur-xl shadow-xl shadow-black/5 p-5 space-y-4 transition-all hover:border-amber-500/30">
    <div className="flex items-start justify-between">
      <div className="flex flex-col gap-1 min-w-0">
        ...
        <Link
          ...
          search={{ email: record.candidate_email }}
          state={{
            interview_status: record.interview_status,
            feedback: record.feedback,
          } as any}
          preload="intent"
          className="flex items-center gap-1.5 mt-1 text-sm font-bold tracking-tight truncate hover:text-indigo-600 dark:hover:text-indigo-400 hover:underline transition-colors"
        >
    ...
    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border/20">
      <div className="flex flex-col gap-1">
        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-center sm:text-left">
          Evaluation
        </span>
    ...
    <div className="flex items-center justify-between pt-2">
      <div className="flex items-center gap-3">
        <div className="flex flex-col gap-0.5">
          <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/30 leading-none">
            Processed
          </span>
          <div className="flex items-center gap-1 text-[10px] text-muted-foreground font-medium opacity-60">
            ...
            <span>
              {record.processed_at
                ? format(record.processed_at, 'MMM dd')
                : 'Pending'}
            </span>
          </div>
        </div>
