...
function ImportContent() {
  ...
  const totalRagFiles = useMemo(() => {
    return processedIndexFiles?.reduce((acc, item) => {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-2xl md:text-4xl font-black tracking-tight text-foreground">Archive Bank</h1>
              <Badge variant="outline" className="hidden sm:inline-flex text-[10px] font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20 px-2 py-0.5">
                Connected
              </Badge>
            </div>
            <p className="text-xs md:text-sm text-muted-foreground font-medium flex items-center gap-1.5 mt-1">
              ...
              <span>Storage repository with {totalBucketFiles} total files across {root_folders.length} deployment batches.</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-border/40">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs font-medium text-muted-foreground/70 ml-2">
              Showing <strong className="text-foreground">{totalFilteredFiles}</strong> of {totalBucketFiles} files
            </span>
          </div>
        </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 md:p-6 bg-muted/20 border-b border-border/30">
                    <div className="flex items-center gap-3.5 min-w-0">
                      <div className={cn(
                        ...
                        {isOpen ? <FolderOpen className="h-5 w-5" /> : <FolderIcon className="h-5 w-5" />}
                      </div>
                        ...
                        >
                          {isFolderLoading ? (
                            ...
                            <>
                              ...
                              <span>{folder.isIndexed ? "Re-index" : "Trigger Index"}</span>
                            </>
                          )}
                        </Button>
                      )}
                          ...
                          >
                            ...
                            <div className="pt-3 mt-3 border-t border-border/30 flex items-center justify-between gap-2">
                              <Button
                                ...
                                onClick={() => handleDownloadUrl(file.full_path, file.name)}
                                disabled={isDownloading || downloadingPath !== null}
                              >
                                {isDownloading ? (
                                  <>
                                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center space-y-4">
              ...
              <div className="space-y-1 max-w-sm">
                <h2 className="text-lg font-bold tracking-tight text-foreground">
                  {searchQuery || statusFilter !== 'all' ? "No matching files or batches found" : "Archives Empty"}
                </h2>
