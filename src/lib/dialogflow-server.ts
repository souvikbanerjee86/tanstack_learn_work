...
export const interactWithAgent = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const authClient = await auth.getIdTokenClient(
      API_PATH.SPEECH_TO_TEXT.GET_BASE_URL,
    )
    ...
    if (textInput) {
      ...
      const url =
        API_PATH.SPEECH_TO_TEXT.GET_BASE_URL + API_PATH.SPEECH_TO_TEXT.PATH_URL
      ...
      const returnResponse = speechResponse.data as {
        text: string | null
      ...
      queryInput = {
        text: {
          text:
            returnResponse.text!.length > 250
              ? 'CHECK_PARAMS'
              : returnResponse.text,
        },
        languageCode,
    ...
    return {
      ...
      audio: response.outputAudio
        ? response.outputAudio.toString('base64')
        : null,
    }
  })
